...
int error;		/* error value returned*/
	...
	if (PFfreebpage != NULL){
		...
		*bpage = NULL;		/* set initial return value */
		if(ALGO == LRU || ALGO == FIFO || ALGO == BELADY)
		{
		...
		if(ALGO == LFU)
		{
			int lowestfreq = 999999;
			PFbpage* lowest =NULL;
			for (tbpage=PFfirstbpage;tbpage!=NULL;tbpage=tbpage->nextpage){
				if (!tbpage->fixed)
					{
						if(tbpage->frequency < lowestfreq)
						{
							lowestfreq = tbpage->frequency;
							lowest = tbpage;
						}
					}
			}	
		...
		if(ALGO == RANDOM)
		{
			int unfixed = 0;
			for (tbpage=PFfirstbpage;tbpage!=NULL;tbpage=tbpage->nextpage){
				if (!tbpage->fixed) unfixed++;
			}
			if(unfixed == 0){tbpage = NULL;}
			else
				{
					// srand(time(NULL));
					int randomnumber = time(NULL)% unfixed;
					int present = 0;
					for (tbpage=PFfirstbpage;tbpage!=NULL;tbpage=tbpage->nextpage){
						if (!tbpage->fixed)
						{
...
int error;
	...
	if ((bpage=PFhashFind(fd,pagenum)) == NULL){
		...
		// printf("%s\n","see here" );
		*fpage = &bpage->fpage;
		PFerrno = PFE_PAGEFIXED;					// TODO: check if frequency should be incremented
		return(PFerrno);
	}
	...
	ACCESS++;
	bpage->fixed = TRUE;
	// printf("On fixing value becomes : %d\n", bpage->fixed);
	*fpage = &bpage->fpage;
	bpage->frequency ++;
	return(PFE_OK);
}
...
PFbpage *bpage;
	...
	// printf("Check 0 : PFbufUnfix :: fd : %d pagenum : %d fix value : %d\n", fd, pagenum, bpage->fixed);
	if (!bpage->fixed){
		/* page already unfixed */
		// printf("Check 1 : PFbufUnfix :: fd : %d pagenum : %d fix value : %d\n", fd, pagenum, bpage->fixed);
...
PFbpage *bpage;
	...
	// printf("before bpage with fd: %d pagenum: %d fixed ? %d\n", bpage->fd, bpage->page, bpage->fixed);
	/* fix the page */
	bpage->fixed = TRUE;
	
	// printf("after bpage fixed ? %d\n", bpage->fixed);
...
int PFbufList(fd_list, page_list)
int * fd_list;
int * page_list;
/****************************************************************************
SPECIFICATIONS:
	Returns the size of the buffer along with the
	list of fd and page number of the pages in buffer.

AUTHOR: manu

*****************************************************************************/
{
	PFbpage *bpage = PFfirstbpage;
	int count = 0;
	for(bpage = PFfirstbpage; bpage != NULL; bpage= bpage->nextpage){
		// if((int)bpage->fixed == 0) continue;
		// printf("Count : %d\n", count);
		fd_list[count] = bpage->fd;
		// printf("problem in fd_list\n");
		page_list[count] = bpage->page;
		// printf("problem in page_list\n");
		count ++;
		// printf("Problem in count\n");
	}
	return count;
	// printf("Check it\n");
	//*list_size = count;
	// printf("Yo ! Done with PFbufList\n");
}

PFBufRelease(writefcn)
int (*writefcn)();	/* function to write a page of file */
/****************************************************************************
SPECIFICATIONS:
	Release all pages of file "fd" from the buffer and
...
int error;		/* error code */
	...
	while (bpage != NULL){
		// if (bpage->fd == fd){
			...
			if (bpage->fixed){
				PFerrno = PFE_PAGEFIXED;
				printf("%s. Page belongs to fd:%d\n", "inside bufrelease, page is fixed", bpage->fd);
				return(PFerrno);
			}

			/* write out dirty page */
			if (bpage->dirty&&((error=(*writefcn)(bpage->fd,bpage->page,
					&bpage->fpage))!= PFE_OK))
				/* error writing file */
				return(error);
			bpage->dirty = FALSE;
			...
			PFbufUnlink(temppage);
			PFbufInsertFree(temppage);
			// printf("successfully removed from hashtable et al\n");
		// }
		// else	bpage = bpage->nextpage;
	}
	return(PFE_OK);
