...
void PF_BufRelease()
{
	// printf("Trying to clear the buffer\n");
	PFBufRelease(PFwritefcn);
}
...
PFfpage *fpage;	/* pointer to file page */
	...
	for (temppage= *pagenum+1;temppage<PFftab[fd].hdr.numpages;temppage++){
		if ( (error=PFbufGet(fd,temppage,&fpage,PFreadfcn,
					PFwritefcn))!= PFE_OK){
			// {printf("	gnf1.1,fd = %d, pagenum = %d, errval=%d\n", fd, *pagenum, error);
			return(error);}
...
PF_FixPage(fd,pagenum)
int fd;	/* file descriptor */
int pagenum;	/* page number */
/****************************************************************************
SPECIFICATIONS:
	Tell the Paged File Interface that the page numbered "pagenum"
	of the file "fd" is very much needed in the buffer.
...
int PF_bufList(fd_list, page_list)
int * fd_list;
int * page_list;
/****************************************************************************
SPECIFICATIONS:
	Get the list of fd and page numbers
...
*****************************************************************************/
{
	return PFbufList(fd_list, page_list);
}
