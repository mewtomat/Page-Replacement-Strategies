...
int fd1,fd2;
	...
	int operation_count = 2*FILE_PAGE_COUNT*FILE_PAGE_COUNT; 
	int page_order[operation_count];
	int count = 0;
	for(i = 0; i<FILE_PAGE_COUNT; i++) {
		int j;
		for(j=0; j<FILE_PAGE_COUNT; j++) {
			page_order[count] = i; count ++;
			page_order[count] = 100+j; count ++;
		}
	}
	...
	*/
	...
	for (i=0; i < FILE_PAGE_COUNT; i++){
		...
		for (j=0; j < FILE_PAGE_COUNT; j++){
			...
			//if(i==12 && j==0) printf("misses = %d accesses = %d\n",MISS, ACCESS);

			if(ALGO == BELADY){
				if(page_order[i*2*FILE_PAGE_COUNT + 2*j] != i) {
					printf("Pain\n");
					return;
				}
				if(page_order[i*2*FILE_PAGE_COUNT + 2*j + 1] != j+FILE_PAGE_COUNT) {
					printf("Pain\n");
				}
				mygetpage(fd1, i, &buf, page_order, i*2*FILE_PAGE_COUNT + 2*j, operation_count);	
				mygetpage(fd2, j, &buf, page_order, i*2*FILE_PAGE_COUNT + 2*j + 1, operation_count);
				continue;
			}
	...
	// printf("Trying to unfix farthest\n");
	// unfix_farthest(page_order, i, operation_count);
	// printf("Unfixed farthest\n");
